...
> 历史计划（非当前执行清单）：E0004–E0007 实际运行使用 Huber。E0008 显式使用 MSE，E0009–E0012 又使用历史 Huber，E0013 及后续实验应逐项核对各自冻结配置与 checkpoint。当前进度见[项目状态](project_status.md)；本文件保留原始问题与假设以便追溯。

> 对照解释注意：E0008 不只替换输出头，还将 Huber 改成 MSE；与 E0004 的现有结果不能单独归因于 decoder。
...
根据后续统一损失协议，E0008 配置将预测损失显式设为 plain MSE；这只替换已暂停使用的 Huber 形式，不改变 direct-edge decoder 对照的结构变量。Huber 留待最终鲁棒性优化阶段。

如果 direct head 在验证集带来稳定改善，再把它扩展到其余架构，形成完整的 decoder 因子比较。E0008 已完成 seed 42 的验证实验；是否扩展到其余架构仍需先依据验证集动态审计结果作出结论。
